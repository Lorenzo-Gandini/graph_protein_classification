...
from models import (EdgeVGAE, GCODWrapper)
from utils import set_seed
from data_loader import create_dataset_from_dataframe
from losses import (GCODLoss, SymmetricCELoss, GeneralizedCELoss)
...
class ModelTrainer:
    def __init__(self, config: 'ModelConfig', device: str):
        ...
        elif self.config.loss_type == 'gce':
            self.criterion = GeneralizedCELoss()
        elif self.config.loss_type == "gcod":
            self.criterion = GCODLoss(self.config.num_classes, self.config.batch_size, self.device)
        else:
    ...
    def evaluate_model(self, model: torch.nn.Module, data_loader: DataLoader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for data in data_loader:
                ...
                if(self.config.loss_type=="gcod"):
                    z, mu, logvar, class_logits = model.gnn(data.x, data.edge_index, data.edge_attr, data.batch)
                    current_uB = model.uB[data.idx]
                    labels = data.y
                    with torch.no_grad():
                        preds = torch.argmax(class_logits, dim=1)      # predicted class indices
                    ...
                    loss, l1, l2, l3 = self.criterion(logits=class_logits, 
                                                uB=current_uB,
                                                labels=labels, 
                                                train_acc=train_acc)
                else:
    ...
    def train_single_cycle(self, cycle_num: int, train_data, val_data):
        ...
        if(self.config.loss_type == "gcod"):
            model = GCODWrapper(model, len(train_data))
        ...
        if(self.config.loss_type == "gcod"):
            optimizer = torch.optim.Adam(model.gnn.parameters(), lr=self.config.learning_rate)
            ub_optimizer = torch.optim.Adam([model.uB], lr=self.config.ub_lr)
        else:
        ...
        for epoch in range(self.config.epochs):
            ...
            model.train()
            if(self.config.loss_type=="gcod"):
                train_loss = self.train_epoch(model, train_loader, optimizer, ub_optimizer)
            else:
            ...
            if (epoch - epoch_best) > self.config.early_stopping_patience//2 and epoch%10==0:
                ...
                if(self.config.loss_type=="gcod"):
                    model.gnn.load_state_dict(model_data['model_state_dict'])
                else:
                    model.load_state_dict(model_data['model_state_dict'])
    ...
    def train_epoch(self, model, train_loader, main_optimizer, ub_optimizer=None):
        model.train()
        total_loss = 0.0
        total_samples = 0
        ...
        for data in train_loader:
            ...
            if self.config.loss_type == "gcod":
                ...
                # GCOD: use wrapped forward model and uB-aware loss
                logits = class_logits
                current_uB = model.uB[data.idx]
                ...
                # Unpack GCOD loss (assumed same format as your earlier GCODLoss)
                loss, l1, l2, l3 = self.criterion(logits=logits, uB=current_uB, labels=data.y, train_acc=acc)

                recon_loss = model.gnn.recon_loss(z, data.edge_index, data.edge_attr)
                kl_loss = model.gnn.kl_loss(mu, logvar)

                total_main_loss = l1 + l3 + 0.15 * recon_loss + 0.1 * kl_loss
                total_main_loss.backward(retain_graph=True)
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
                main_optimizer.step()

                # --- GCOD uB optimization step ---
                with torch.no_grad():
                    z, mu, logvar, class_logits = model.gnn(data.x, data.edge_index, data.edge_attr, data.batch)  # GNN forward again
                    logits = class_logits
                current_uB = model.uB[data.idx].requires_grad_()
                probs = F.softmax(logits, dim=1)
                yB = F.one_hot(data.y, num_classes=self.config.num_classes).float()
                adjusted_probs = probs + torch.diag(current_uB) @ yB
                L2 = torch.norm(adjusted_probs - yB, p=2)**2 / self.config.num_classes
                ...
                loss = total_main_loss + L2  # Optional: include for logging

            else:
                z, mu, logvar, class_logits = model(data.x, data.edge_index, data.edge_attr, data.batch)
                # Standard loss (non-GCOD)
    ...
    def prepare_data_split(self, df, seed=1):
        ...
        if len(db_lst) > 1:  # Same split per individual dataset
            ...
            for x in db_lst:
                ...
                sub_df = df.loc[idx, :]
                labels = sub_df['y'].values  # Adjust if label column is named differently
                ...
                df_train = pd.concat([df_train, df.loc[train_idx]], ignore_index=True)
                df_valid = pd.concat([df_valid, df.loc[valid_idx]], ignore_index=True)
